...
public class ProjekSDA4 {
    ...
    static void findPrim(String start) {
        boolean found = false;
        for (int i = 0; i < edgeCount; i++) {
            if (edgeFrom[i].equals(start) || edgeTo[i].equals(start)) { 
                found = true; 
                break; 
            }
        }
        if (!found) { 
            System.out.println("Start building not found."); 
            return; 
        }

        String[] visited = new String[256];
        int visitedCount = 0;

        String[] parentKey = new String[256];
        String[] parentVal = new String[256];
        int parentCount = 0;

        String[] minWeightKey = new String[256];
        int[] minWeightVal = new int[256];
        int minWeightCount = 0;

        for (int i = 0; i < edgeCount; i++) {
            boolean existsFrom = false;
            for (int j = 0; j < minWeightCount; j++) {
                if (minWeightKey[j].equals(edgeFrom[i])) { existsFrom = true; break; }
            }
            if (!existsFrom) {
                minWeightKey[minWeightCount] = edgeFrom[i];
                minWeightVal[minWeightCount] = Integer.MAX_VALUE;
                minWeightCount++;
            }
            boolean existsTo = false;
            for (int j = 0; j < minWeightCount; j++) {
                if (minWeightKey[j].equals(edgeTo[i])) { existsTo = true; break; }
            }
            if (!existsTo) {
                minWeightKey[minWeightCount] = edgeTo[i];
                minWeightVal[minWeightCount] = Integer.MAX_VALUE;
                minWeightCount++;
            }
        }
        ...
        for (int i = 0; i < minWeightCount; i++) {
            if (minWeightKey[i].equals(start)) {
                minWeightVal[i] = 0;
                break;
            }
        }
        ...
        System.out.println("\n=== MST Result - Minimum Connections (Prim) ===");

        while (!queue.isEmpty()) {
            int curEdgeWeight = queue.peekDist();
            String curBuilding = queue.extractMin();

            boolean isVisited = false;
            for (int i = 0; i < visitedCount; i++) {
                if (visited[i].equals(curBuilding)) { isVisited = true; break; }
            }
            if (isVisited) continue;

            visited[visitedCount++] = curBuilding;
            totalMSTWeight += curEdgeWeight;

            // Print the edge used to discover this node (skip for the starting node)
            for (int i = 0; i < parentCount; i++) {
                if (parentKey[i].equals(curBuilding)) {
                    System.out.println("  " + parentVal[i] + " -- " + curBuilding + " == " + curEdgeWeight);
                    break;
                }
            }
            ...
            for (int i = 0; i < edgeCount; i++) {
                ...
                if (edgeFrom[i].equals(curBuilding)) {
                    neighbour = edgeTo[i];
                } else if (edgeTo[i].equals(curBuilding) && edgeOneWay[i] == 0) {
                    neighbour = edgeFrom[i];
                }
                if (neighbour == null) continue;

                // Check if neighbor is already in the MST
                boolean neighborVisited = false;
                for (int j = 0; j < visitedCount; j++) {
                    if (visited[j].equals(neighbour)) { neighborVisited = true; break; }
                }
                if (neighborVisited) continue;

                // Get current best structural cost to connect this neighbor
                int currentBestWeight = Integer.MAX_VALUE;
                int weightIndex = -1;
                for (int j = 0; j < minWeightCount; j++) {
                    if (minWeightKey[j].equals(neighbour)) {
                        currentBestWeight = minWeightVal[j];
                        weightIndex = j;
                        break;
                    }
                }

                // PRIM CRITICAL DIFFERENCE:
                // We compare against the edge weight itself, NOT accumulated distance!
                int edgeCost = edgeWeight[i]; 
                if (edgeCost < currentBestWeight) {
                    minWeightVal[weightIndex] = edgeCost;

                    // Update parent tracking pointer
                    boolean parentUpdated = false;
                    for (int j = 0; j < parentCount; j++) {
                        if (parentKey[j].equals(neighbour)) {
                            parentVal[j] = curBuilding;
                            parentUpdated = true;
                            break;
                        }
                    }
                    if (!parentUpdated) {
                        parentKey[parentCount] = neighbour;
                        parentVal[parentCount] = curBuilding;
                        parentCount++;
                    }

                    queue.insert(neighbour, edgeCost);
                }
            }
        }
    ...
    public static void main(String[] args) {
        ...
        while (true) {
            ...
            if (choice.equals("6")) { System.out.println("Terimakasih!"); break; }
